...
description: Use only when the user explicitly requests the autonomous variant of end-to-end-coding-session and consents to a persistent Codex goal plus automatic commit on an isolated branch. Fresh adversarial Terra reviewers approve the plan and final diff, the user still chooses once whether to run heavy Peer Bug Review, and the workflow never auto-pushes or merges. Do not infer this mode from a generic request to implement, move fast, or work end to end.
---
...
The outer session owns routing, the living plan, the Codex goal, reviewer
iterations, `terminal_peer_review_state`, and the branch commit.
...
Replace its goal-timing and stop-before-commit disclosure with this Automatic
consent contract:

- The user selected Automatic.
- Eligibility will be checked read-only after alignment; only an eligible task
  creates one persistent Codex goal.
...
eligibility-before-goal creation, and isolated-branch auto-commit authority. A
...
If eligible, create the goal. Record outcome, in/out scope, acceptance evidence,
`workflow_owner=end-to-end-coding-session-automatic`, the canonical
`living_plan`, and the auto-commit `terminal_contract`: accepted plan, accepted
final diff, resolved heavy-review choice, verified isolated-branch commit. Never
set a token budget unless the user supplied one. Record the eligibility result
and evidence in the living plan.
...
it plus the authorized objective to a fresh, no-context Terra reviewer.
...
Limit: three plan-gate attempts. A repeated intent/authority blocker routes to
the user and human-gated workflow; do not self-approve.
...
and verification evidence. Dispatch a fresh Terra reviewer.
...
Call `update_goal(complete)` only after the commit exists, mapped verification is
green or honestly bounded, the final code gate accepted, the heavy-review choice
resolved, and the branch/worktree state is reported.

Call `update_goal(blocked)` only after the same real blocker recurs for at least
three consecutive goal turns. `terminal_peer_review_state=blocked` is separate
workflow state and does not by itself authorize blocking the goal.
...
| Plan approval | 3 fresh reviewers | Stop with blockers |
...
Do not reset counters after compaction. Resume from the goal and living plan.

## Final Response

Report changed behavior, exact verification, eligibility result, plan/code gate
counts, heavy-review state, goal state, commit SHA/message/branch, dirty state,
